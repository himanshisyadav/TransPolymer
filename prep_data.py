...
def exact_paper_seq_ood_using_all_solvs_temp(num_file, smiles_file, output_file):
    df_num = pd.read_csv(num_file)
    df_smiles = pd.read_csv(smiles_file)
    df_smiles.fillna('NAN_SMILES', inplace=True)
    df_input = pd.concat([df_num, df_smiles], axis = 1)
...
def main():
    ood_scaled_fusion("./data/cond_ood_final_add.csv", "./data/cond_ood_final_comp.csv", "./data/cond_ood_exact_paper_seq_scaled_temp_sep_no_mol.csv")
